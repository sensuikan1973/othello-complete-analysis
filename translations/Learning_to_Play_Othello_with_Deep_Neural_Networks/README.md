...
ドロップアウトは、訓練例の個々のバッチに対してランダムに数のユニット(ここでは50%)を無効にすることで構成されます。  
...
訓練セットの分類精度は、通常、テストセットの精度の 1 % ポイント以内です。これは、ここで検討したネットワークでは、訓練データを完全に記憶できないことを示しています。  
...
CNN の力は、その知識を抽出して、訓練の例にとどまらずに一般化されたシステムとして表現できるところにあります。  
