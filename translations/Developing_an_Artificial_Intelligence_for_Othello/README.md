...
αβ は、Minimax 探索の大きな改善版であり、同等の探索時間のまま、探索深度で劇的に改善することができます。
...
Prob-cut と Multi Prob-cut のバリエーションは、注意深く調整すると、探索ツリーの大幅な刈り込みに繋がり、より深い探索が可能になります。
...
伝統的なオセロ(またはチェスや囲碁)のプログラムは、αβツリー探索と評価関数を組み合わせてゲームの着手を予測し、特定の位置あるいは着手のスコアを推定します。
...
これは2つの方法で行うことができます:
独自の αβ 探索および評価関数でそれらの局面を評価するために参考プログラムを使用するか、ゲームのデータベースで統計を作成し、ゲームツリーの minimax 順序で各位置の値を推定するか です。
...
オセロ(およびチェスや囲碁)などのゲームに適用される特定のモンテカルロ法の 1 つは、従来の αβ ツリー探索の代わりに使用できるモンテカルロツリー探索(MCTS)です。

最適なブランチのスコアについてツリー構造を探索するMCTSのアイデアは、ブランチのサブセットをランダムに探索し、探索されたブランチのスコアの平均が最適なスコアになると想定することです。
...
SIMD 命令(Intel / AMD CP Uの AVX2)は間違いなく役立ちますが、ニューラルネットワークはワークロードの少なくとも 80 %を占め、残りは αβ ツリー探索中に着手して進行するためのビットボード操作です。
