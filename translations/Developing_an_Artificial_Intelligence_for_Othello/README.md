...
1. 訓練フェーズ。既知の入力と結果の組み合わせのデータセットが、各ニューロンとその活性化関数のパラメータを反復的に改良する訓練アルゴリズムが用いられます。この手法は、ニューラルネットが模倣するために学習する、既知のデータセットが必要なので、"教師あり学習" として知られています。
2. 予測フェーズ。ニューラルネットが 1 で得た知識を、1 では見たことない新しい入力に対して適用し、結果を予測します。

訓練フェーズでは、多くの "タグ付き" サンプル、つまり出力が分かっている入力のサンプルが必要になります。
Neural networks にニューロンが多いほど、訓練サンプルの量も多く必要になります。
...
このアプローチのもう一つの欠点は、訓練に必要な配置パターン数が非常に多いことです。
私はこのアプローチをすぐに諦めました。
...
埋め込み層の係数は数が非常に多いですが、訓練時に計算されます。動的に入力の次元数を削減することで、埋め込み層は、最初の密な隠れ層の乗算数を制限します。
...
* 過学習とは、ニューラルネットワークが大きすぎてパラメーターが多すぎるために、全ての具ｙん蓮データを期待される出力とともに保存できるメモリバンクになる一方で、未知のデータに対して適切な予測を行うことができない状態です。解決策は、ネットワークを単純化する(より狭い層やより少ない隠れ層)、ネットワークが全てを記憶できないような訓練データを追加する、または "ドロップアウト" や正則化などの手法を使用することです。
